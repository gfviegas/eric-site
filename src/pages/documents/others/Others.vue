<template lang="pug">
  div.scout-method
    div.content-block
      h3.title.is-3 OUTROS DOCUMENTOS
      div.list-documents
        p.control
...
<script>
  ...
  export default {
    head: {
      title: getSeoTitle('Outros Documentos'),
      meta: () => {
        return getSeoMeta({
          title: 'Outros Documentos - Escoteiros de Minas',
          description: 'Acesse as resoluções dos Escoteiros de Minas Gerais'
        })
      }
    },
    beforeRouteEnter (to, from, next) {
      documentsService.get({type: 'other'}).then((response) => {
        next((vm) => {
