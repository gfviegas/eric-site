...
<script>
  ...
  export default {
    head: {
      title: getSeoTitle('Atas'),
      meta: () => {
        return getSeoMeta({
          title: 'Atas - Escoteiros de Minas',
          description: 'Acesse as atas dos Escoteiros de Minas Gerais'
        })
      }
    },
    beforeRouteEnter (to, from, next) {
      documentsService.get({type: 'minute'}).then((response) => {
        next((vm) => {
