          ...
          div.control
            div.select.is-fullwidth
              select(name="form.reward" v-model="form.reward" v-validate="validations.reward" data-vv-as="Distintivo Solicitado" v-bind:class="{'is-danger': errors.has('form.reward')}")
                ...
                option(value="Nível Avançado") Avançado
                option(value="Nível Básico") Básico
                option(value="Nível Dirigente") Dirigente
                option(value="Nível Escotista") Escotista
        div.field
...
<script>
  ...
  export default {
    ...
    methods: {
      submitForm () {
        this.$ga.trackEvent('Requests Form', 'Submit', 'Level')
        this.submitted = true
    ...
    head: {
      title: getSeoTitle('Solicitação de Nível'),
      meta: () => {
        return getSeoMeta({
          title: 'Formulário de Solicitação de Nível - Escoteiros de Minas',
          description: 'Preencha o formulário de solicitação de nível - Escoteiros de Minas Gerais'
        })
      }
    },
    created () {
